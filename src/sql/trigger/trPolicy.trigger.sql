DROP TRIGGER IF EXISTS dbo.trPolicy
GO

---
-- Trigger for Policy that will handle logging of both update and delete
-- NOTE: inserted data is current value in row if not altered
---
CREATE TRIGGER trPolicy
	ON [dbo].[Policy]
	FOR UPDATE, DELETE
	AS
		BEGIN
	...
	values('Policy', 'dbo', @action, CASE WHEN LEN(HOST_NAME()) < 1 THEN ' ' ELSE HOST_NAME() END,
